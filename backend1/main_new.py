...
def resolve_optional_path(*candidates: Path) -> Optional[Path]:
    for candidate in candidates:
        if candidate.exists():
            return candidate
    return None
...
class_indices = np.load(
    str(resolve_existing_path(DATASET_DIR / "class_mapping.npy", API_DIR / "class_mapping.npy")),
    allow_pickle=True,
).item()
...
def index() -> HTMLResponse:
    return HTMLResponse(
        ...
        <!DOCTYPE html>
        <html lang="en">
        <head><meta charset="UTF-8" /><title>ISL Translator Backend</title></head>
        <body>
            <h1>Backend running</h1>
            <p>Use the frontend on http://localhost:5173</p>
            <p>Video stream endpoint: <a href="/video_feed">/video_feed</a></p>
        </body>
        </html>
        """
    )
